...
extension PhoneEntryViewModel {
    ...
    mutating func setDeviceLocale() {
        ...
        self.localeCountry = countries.countries.filter {($0.iso2cc == countryCode)}.first
        self.countryCodeTxt.value = localeCountry.iso2cc + " " + "(+" + localeCountry.e164cc + ")"
    }
    
    func send(code: String, completionHandler: @escaping (Result<String,Error>) -> Void) {
        let phoneNumber = "+" + localeCountry.e164cc + code
